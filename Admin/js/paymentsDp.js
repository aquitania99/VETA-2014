...
function previousReceipt(receiptNo) {
	...
	var sendData = {'check': 1, 'receipt_number': receiptNo, 'personID': personID, 'pID': pID, 'courseEntry': courseEntry, 'courseType': courseType};
	//
	$.ajax({
		type: "POST",
		...
		success: function (data) {
			...
			//window.location = 'createStudentReceipt.php?keyVal='+personID+'&receiptNumber='+receiptNo+'&courseEntry='+courseEntry+'&pID='+pID;
			window.open('createStudentReceiptDp.php?keyVal=' + personID + '&receiptNumber=' + receiptNo + '&courseEntry=' + courseEntry + '&pID=' + pID + '&cType=' + courseType);
		}
	});
}
...
function paymentReceipt(inst) {
	...
	//
	var sendData = {'courseEntry': courseEntry, 'courseType': courseType, 'pID': pID, 'personID': personID, 'amountDue': amountDue, 'amountPaid': amountPaid, 'amountOutstanding': amountOutstanding, 'paymentComments': receiptNotes, 'receivedBy': receivedBy};
	$.ajax({
		type: "POST",
		url: "classes/PaymentClass.php",
		data: sendData,
