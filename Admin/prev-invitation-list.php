...
//DATE_FORMAT(DATE '%d/%m/%Y') as formatted_date
	$queryPrevious = $db->dbQuery("SELECT seminarID as 'ID', date as 'heldOn', title1 as title, creationDate as 'Date'  FROM seminars ORDER BY Date DESC LIMIT 0, 10");
	$rowsPrevious = $db->fetch_array($queryPrevious);
/// Classifieds Details
	//$email_ID = $rowsPrevious['ID'];
	//$title_NW = $rowsPrevious['title1'];
//////////////////////////////////////////	
...
///        
	...
	<head>
		...
		<title>Previous Newsletters :: Australia VETA</title>
	...
	<table width="610" border="0" align="center" cellpadding="4" cellspacing="4" bgcolor="#FFFFFF" class="bordes">
		...
		<tr>
			<td align="center" valign="middle"><h2><span
						title="Click for alternate translations">Seminar/Invitations</span> <br/>
		...
		do {
			...
			<tr>
				<td style="padding-left:18px;">
					<font style="color:mediumvioletred;">
						<a href="newsletter/previewSeminars.php?ID=<?php echo $rowsPrevious['ID']; ?>"
						   title="Click to preview this invitation created on <?php echo date("l jS \of F Y h:i:s A", strtotime($rowsPrevious['Date'])); ?>"><?php echo $num . ")&nbsp;" . $rowsPrevious['title'] . " - " . date("l jS \of F Y h:i:s A", strtotime($rowsPrevious['Date'])); ?></a></font>
				</td>
			</tr>
		<?php } while ($rowsPrevious = $db->fetch_array($queryPrevious)); ?>
		<tr>
			<td>&nbsp;</td>
		</tr>
	</table>
