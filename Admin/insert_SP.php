...
//
$vs_CrsName = $_POST['courseName'];
//
$vs_courseName = $vs_CrsName;
//
...
$db->open();
/*** Prev - Check START ***/
$prevCheck = $db->dbQuery("SELECT count(1) as count FROM `persons` WHERE emailAddress = '$p_emailAddress'");
...
IF ($res > 0) {
	...
	if ($maxCountIdBefore >= 1) {
        ...
        enrolment_date = '$vs_enrolmentDate', course_lenght = $vs_courseLenght, genComments = '%s'
        WHERE emailAddress = '$p_emailAddress'");
		sprintf(mysql_real_escape_string($vp_genComments));
	} else {
        ...
        '$vs_enrolmentDate',$vs_courseLenght, '%s')");
		sprintf(mysql_real_escape_string($vp_genComments));
	}
	///
		...
		*/
		$insertInv = $db->dbQuery("INSERT INTO invoices (invoice_Id, studentID, emailAddress, edu_provider_ID, paymentFee, commissionRate, commissionValue, GSTexc, GSTinc, invoiceNumber, CommDeducted, marketingIncentive, mkIncentiveValue,
          ...
          VALUES('','" . $p_passNumber . "','" . $p_emailAddress . "','" . $vs_eduCentre . "', '" . round($n, 2) . "', '" . $commRate . "', '" . round($realPay[$key], 2) . "', '" . round($exc[$key], 2) . "', '" . round($inc[$key], 2) . "', '" . $invNumber[$key] .
			"', '" . $CommDeducted[$key] . "','" . $marketingIncentive[$key] . "','" . $mIncentive[$key] . "', '" . $payDateDue[$key] . "', '" . $StudentPaidDate[$key] . "', '" . round($totalPaid[$key], 2) . "', '" . $ColPaymentDateDue[$key] . "', '" . $collegePaidDate[$key] . "', '" . $ColTotalPaid[$key] . "'); ");
		//reset($results);
...
//////////////////////////////////       
	$queryInsert = $db->dbQuery("INSERT INTO persons (creationDate, statusID, stCounsellor,
	...
	$queryMaxIdAfter = $db->dbQuery("SELECT max(personID) as idAfter FROM `persons`");
