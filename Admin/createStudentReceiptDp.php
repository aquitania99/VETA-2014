...
//
//var_export($paymentResult);die;
date_default_timezone_set("Australia/Sydney");
$date = date('d/m/Y');
$today = $_POST['today'];
$sql_select = "SELECT * FROM payment_received WHERE person_ID LIKE '$keyVal' AND receipt_number = '$receiptNumber' AND courseEntry = $courseEntry AND prevID = $pID  ORDER BY paid_on DESC ";
...
$courseInstalment = $paymentResult['instalment'];
...
//$collegeId == $college; //= (($courseEntry == 1) ? $paymentResult['college'] : $paymentResult['college'.$courseEntry]);
//
//print_r($college);
...
$searchColleges .= 'WHERE edu_providerID IN (' . $college . ')';
//
...
$collegeName->college1 = $college;
//print_r($collegeName->college1); 
$collegeName->search();
//
$name = $collegeName->storage;
//
*/
//var_dump($name);die('BUUUAAAAJAJAJAJ!!');exit;
