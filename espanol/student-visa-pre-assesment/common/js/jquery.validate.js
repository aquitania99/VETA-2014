(function(a) {
	...
	a.extend(a.validator, {
		...
		messages : {
			...
			email : "Por favor digite una dirección de correo válida.",
			...
			range : a.validator.format("Please enter a valid value between {0} and {1}."),
		...
		normalizeRules : function(c, b) {
			...
			a.each(["rangelength", "range"], function() {
				if (c[this]) {
					if (c[this][2]) {
						c[this] = [Number(c[this][0]), Number(c[this][1]), Number(c[this][2])]
					} else {
		...
		methods : {
			...
			range : function(c, b, e) {
				var d = true;
				if (e.length > 2) {
					d = (parseFloat(c - e[0]).toFixed(6) * Math.pow(10, 6)).toFixed(6) % (e[2] * Math.pow(10, 6)) == 0
				}
				return this.optional(b) || (c >= e[0] && c <= e[1] && d)
			},
			email : function(c, b) {
			...
			date : function(c, b) {
				return this.optional(b) || !/Invalid|NaN/.test(new Date(jQuery.datepicker.parseDate(jQuery(b).datepicker("option", "dateFormat"), c)))
			},
